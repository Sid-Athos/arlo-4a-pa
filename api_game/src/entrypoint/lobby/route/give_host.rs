...
pub async fn give_host(State(pool): State<ConnectionPool>, Extension(user): Extension<User>, Path(user_id): Path<i32>) -> Result<Json<LobbyMemberResponse>, StatusCode> {
    ...
    let lobby_member = lobby_service.give_host(user.id, user_id).await?;
