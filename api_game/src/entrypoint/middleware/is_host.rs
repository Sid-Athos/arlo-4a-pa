...
pub async fn is_host<B>(State(pool): State<ConnectionPool>, mut req: Request<B>, next: Next<B>) -> Result<Response, StatusCode> {
    ...
    let lobby = lobby_service.get_lobby_member_by_user_id(user.id).await?;
