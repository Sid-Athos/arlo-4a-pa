...
pub fn friend_list_routes(pool: Pool<PostgresConnectionManager<NoTls>>) -> Router {
    ...
    Router::new()
        ...
        .route("/:friend_list_id", delete(delete_friend).route_layer(middleware::from_fn_with_state(pool.clone(), is_logged)))
        .route("/:friend_list_id", put(accept_friend_request).route_layer(middleware::from_fn_with_state(pool.clone(), is_logged)))
        .route("/", get(show_friend_list).route_layer(middleware::from_fn_with_state(pool.clone(), is_logged)))
        .route("/requests", get(show_friend_request).route_layer(middleware::from_fn_with_state(pool.clone(), is_logged)))
        .with_state(pool)
