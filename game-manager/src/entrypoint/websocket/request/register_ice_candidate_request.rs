...
impl RegisterICECandidateRequest {
    pub async fn compute(&self, pool: ConnectionPool, connections: Extension<Connections>, user: User) -> Result<(), String> {
        ...
        let response = ICECandidateResponse::new(self.candidate.clone(), self.sdp_mid.clone(), self.sdp_m_line_index);

        connections.send_to_vec_user_id(ResponseEnum::ICECandidate(response), members_user_id).await;
