...
pub struct SDPAnswerRequest {
    sdp: String,
}

impl SDPAnswerRequest {
    ...
    pub async fn compute(&self, pool: ConnectionPool, connections: Extension<Connections>, user: User) -> Result<(), String> {
        ...
        let response = SDPAnswerResponse::new(self.sdp.clone());

        connections.send_to_vec_user_id(ResponseEnum::SDPAnswer(response), members_user_id).await;
