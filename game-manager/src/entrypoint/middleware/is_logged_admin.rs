...
pub async fn is_logged_admin<B>(State(pool): State<ConnectionPool>, mut req: Request<B>, next: Next<B>) -> Result<Response, StatusCode> {
    ...
    let user = match session_service.get_user_by_token(auth_header.to_string()).await {
        ...
        Err(_) => {
            return Err(StatusCode::UNAUTHORIZED); //TODO a voir avec arnaud pour les erreurs
        },
    };
