...
async fn read(mut receiver: SplitStream<WebSocket>, who: SocketAddr, connections: Connections) {
    while let Some(Ok(msg)) = receiver.next().await {
...
fn process_message(msg: Message, who: SocketAddr) -> ControlFlow<(), ()> {
    match msg {
        ...
        Message::Binary(d) => {
            let us: Request = match deserialize(d) {
                Ok(us) => us,
                Err(_err) => {
                    tracing::debug!("Error deserializing {}", _err);
                    return ControlFlow::Continue(());
                }
            };
            let date_time = Utc.timestamp_millis_opt(us.timestamp).unwrap();
            tracing::info!("Time {:?} ", date_time);

            tracing::info!("Deserialized {:?} {:?}", us.message, us.timestamp);
            return ControlFlow::Break(());
        }
        Message::Close(c) => {
            if let Some(cf) = c {
                tracing::info!(">>> {} sent close with code {} and reason `{}`",
...
pub fn deserialize(d: Vec<u8>) -> Result<Request, serde_json::Error>{
    serde_json::from_str(str::from_utf8(&d).unwrap())
}
